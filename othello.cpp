...
// Parallelized NegaMax Algorithm using Cilk
  ...
  cilk_for(int row = 1; row <= 8; row++) {
      for (int col = 1; col <= 8; col++) {
          if (legal_moves.disks[color] & BOARD_BIT(row, col)) {
              ...
              maxScore.calc_max(score);  // max score tracking
          }
      }
  }
...
int main (int argc, const char * argv[]) 
{
  ...
  do {
    move_possible = 0;
    if(p1 == 'c'){
      move_possible = CompTurn(&gameboard, X_BLACK, d1);
    } 
    else if(p1 == 'h'){
    ...
    if(p2 == 'c'){
      move_possible |= CompTurn(&gameboard, O_WHITE, d2);
    } 
    else if(p2 == 'h'){
